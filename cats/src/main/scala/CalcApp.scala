...
object Serializer extends Calc[Const[String, ?]] {
  ...
  def add(l: Const[String, Int], r: Const[String, Int]): Const[String, Int] = Const(s"(${l.getConst} + ${r.getConst}")
  def sqrt(i: Const[String, Int]): Const[String, Double] = Const(s"sqrt(${i.getConst})")
}
